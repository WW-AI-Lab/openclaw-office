...
- [src/components/office-2d/furniture/Sofa.tsx:10-28](file://src/components/office-2d/furniture/Sofa.tsx#L10-L28)
...
- 办公桌 Desk：扁平等距矩形桌面与前缘深度、显示器提示、键盘提示，支持深色/浅色主题。
- 椅子 Chair：顶视背靠弧线与坐垫圆形，主题色随 isDark 切换。
- 会议桌 MeetingTable：圆形桌面，径向渐变填充，带阴影滤镜。
- 沙发 Sofa：长条框架+坐垫+靠包，支持旋转角度以适配不同朝向。
- 植物 Plant：花盆路径+多片叶子椭圆，营造绿意。
- 咖啡杯 CoffeeCup：托盘+杯身+咖啡液面+杯柄+蒸汽，细节丰富。
- 工位单元 DeskUnit：将 Desk、Chair 与 AgentAvatar 组合，使用 memo 优化渲染。
...
PA-->>MM : 返回等角座位坐标
MM-->>FP : 返回分组+桌心+座位映射
FP->>FP : 渲染 MeetingTable + MeetingChairs
```
...
## 结论
家具装饰系统通过“场景层 + 家具层 + 智能体层”的分层设计，结合自适应布局算法与主题适配机制，实现了在不同区域内的稳定、美观与高性能渲染。DeskUnit 将家具与智能体整合为可复用单元，配合 memo 优化与合理的渲染层级，确保在大规模智能体场景下依然保持流畅体验。未来可在以下方向持续演进：更丰富的家具类型与材质库、动态主题切换的过渡动画、以及针对极端负载的批处理渲染策略。
