...
- 2D 办公室场景位于 src/components/office-2d，包含 Agent 头像、家具、连接线、楼层平面等组件。
...
- 智能体头像系统：基于 agentId 的确定性 SVG 头像生成，支持多种面部形状、发型、眼睛样式与颜色；实时状态动画（空闲/思考/工具调用/回复/错误/创建中）。
...
### 智能体头像系统
- 确定性头像生成：基于 agentId 的哈希，选择面部形状、发型、眼睛样式、肤色、发色与衬衫色，保证同一 agentId 永远生成相同头像。
- 实时状态动画：
  - 思考：脉冲动画
  - 工具调用：带虚线的脉冲
  - 回复：气泡图标脉动
...
- 选中高亮与悬停提示：选中态发光环与信息气泡。

```mermaid
flowchart TD
Start(["进入 AgentAvatar 渲染"]) --> Gen["generateSvgAvatar(agentId)"]
Gen --> DrawFace["绘制脸/头发/眼睛"]
DrawFace --> Status{"当前状态？"}
Status --> |thinking| Pulse["脉冲动画"]
Status --> |tool_calling| DashPulse["虚线脉冲"]
Status --> |speaking| Bubble["气泡脉动"]
Status --> |error| Blink["闪烁"]
Status --> |spawning| Spawn["入场动画"]
Status --> |idle| None["无特殊动画"]
Pulse --> End(["完成"])
DashPulse --> End
Bubble --> End
Blink --> End
...
- 组件耦合：
  ...
  - AgentAvatar 依赖 avatar-generator、constants、office-store。
...
DU["DeskUnit.tsx"] --> AV["AgentAvatar.tsx"]
DU --> FURN["furniture/*"]
AV --> AVGEN["avatar-generator.ts"]
FP --> C["constants.ts"]
```
...
## 结论
本系统通过等距投影与 SVG 渲染，将复杂的 Agent 协作关系与动态状态以直观的 2D 场景呈现。确定性头像、路径规划与会议调度算法确保了可预测的用户体验；模块化的组件与状态管理便于扩展与维护。建议在大规模场景下进一步优化渲染层级与动画频率，以获得更流畅的交互体验。

[本节为总结性内容，不涉及具体文件分析]

## 附录
- 使用模式与最佳实践：
  - 头像生成：使用 generateSvgAvatar 保证跨会话一致性。
